...
def parse_args():
    parser = argparse.ArgumentParser(description="Convert Moxfield deck IDs to Forge .dck files")
...
def extract_deck_sections(data: dict) -> tuple[str, dict, dict, dict, dict]:
    ...
    def process_section(section_data: dict) -> dict:
        """Process a deck section, extracting quantity and layout info."""
        processed = {}
        for card_name, card_data in section_data.items():
            layout = card_data.get("card", {}).get("layout", "")
            processed[card_name] = {"quantity": card_data["quantity"], "layout": layout}
        return processed
...
def create_dck_file(output_path: Path, deck_name: str, commanders: dict, mainboard: dict, sideboard: dict) -> bool:
    """
