...
class RiftCodexBackend(CardBackend):
    ...
    def _release_date(self, set_code: object) -> str:
        set_code = str(set_code).casefold()
        if set_code not in self._set_release_dates:
            data = cast("dict[str, object]", self.request_json(f"https://api.riftcodex.com/sets/set-id/{set_code}"))
    ...
    def _search_by_name(self, name: str) -> list[CardRecord]:
        ...
        data = cast("dict[str, object]", self.request_json(url))
        return cast("list[CardRecord]", data.get("items", []))
