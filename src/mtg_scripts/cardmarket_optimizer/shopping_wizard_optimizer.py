...
# Ideas for criteria:
# First iteration:
# Add groups with at least three cards and reasonable shipping.
# Second iteration:
...
from .common import get_cart_price, handle_alert, parse_number
from .shopping_wizard_optimizer_filter import filters
...
def add_seller_to_cart(driver: WebDriver, seller_name: str) -> None:
...
try:
    accept_button = WebDriverWait(driver, 2).until(
        expected_conditions.element_to_be_clickable(
            (By.XPATH, "//button[normalize-space(text())='Accept All Cookies']")
        )
    )
    accept_button.click()
...
text_box = WebDriverWait(driver, 1).until(
    expected_conditions.element_to_be_clickable((By.XPATH, "//input[@name='userPassword']"))
)
text_box.clear()
text_box.send_keys(args.password)

login_button = WebDriverWait(driver, 1).until(
    expected_conditions.element_to_be_clickable((By.XPATH, "//input[@type='submit' and @title='Log in']"))
)
login_button.click()
account_dropdown = WebDriverWait(driver, 10).until(
    expected_conditions.presence_of_element_located((By.ID, "account-dropdown"))
)
...
while cart_has_items:
    ...
    while not done:
        try:
            ...
            next_button = WebDriverWait(select_wants_list_section, 5).until(
                expected_conditions.element_to_be_clickable((By.XPATH, ".//button[contains(@class, 'next-btn')]"))
            )
    ...
    list_container = WebDriverWait(driver, 5).until(
        expected_conditions.visibility_of_element_located((By.CSS_SELECTOR, "#sellerCountry .list-container.show"))
    )
    ...
    seller_countries = filters.get("sellerCountry", [])
    if not isinstance(seller_countries, list):
        seller_countries = []
    for opt in opts:
        value: int | str | None = opt.get_attribute("data-option-value")
        if str(value).isdigit():
            value = int(str(value))
        if value in seller_countries:
    ...
    seller_types = filters.get("sellerType", [])
    if not isinstance(seller_types, list):
        seller_types = []
    ...
    label = WebDriverWait(strategy_section, 5).until(
        expected_conditions.element_to_be_clickable((By.XPATH, f".//label[@for='{reduce_price_radio_btn_id}']"))
    )
    ...
    run_wizard_button = WebDriverWait(strategy_section, 5).until(
        expected_conditions.element_to_be_clickable((By.XPATH, ".//button[contains(., 'Run Wizard')]"))
    )
    ...
    summary_container = WebDriverWait(driver, 10).until(
        expected_conditions.presence_of_element_located((By.ID, "ShoppingWizardResult"))
    )
    ...
    for dt, dd in zip(dt_elems, dd_elems, strict=False):
        ...
        for seller_name, summary in summaries_per_seller.items():
            ...
            match strategy:
                case 0:
                    ...
                    enough_cards = summary["wanted-articles"] >= FOUR_ARTICLES
                    covers_shipping = summary["articles-value"] >= summary["shipping-cost"]
                    if enough_cards and covers_shipping:
                        add_seller = True
                case 1:
                    # Same as (0) but OR instead of AND (more flexible now).
                    enough_cards = summary["wanted-articles"] >= FOUR_ARTICLES
                    covers_shipping = summary["articles-value"] >= summary["shipping-cost"]
                    if enough_cards or covers_shipping:
                        add_seller = True
                case 2:
                    # Same as (0) but with 2 or more cards and half the shipping cost.
                    enough_cards = summary["wanted-articles"] >= TWO_ARTICLES
                    covers_half_shipping = summary["articles-value"] >= 0.5 * summary["shipping-cost"]
                    if enough_cards and covers_half_shipping:
                        add_seller = True
                case 3:
                    # Same as (2) but OR instead of AND (more flexible now).
                    enough_cards = summary["wanted-articles"] >= TWO_ARTICLES
                    covers_half_shipping = summary["articles-value"] >= 0.5 * summary["shipping-cost"]
                    if enough_cards or covers_half_shipping:
                        add_seller = True
                case _:
                    # No filters. By this point there's not much we can do.
                    add_seller = True
            if add_seller:
                sellers_added += 1
                article_count = len(results_details_per_seller[iteration_num][seller_name])
                print(f"Adding {article_count} articles from seller '{seller_name}' using strategy {strategy}.")
    ...
    checkbox = WebDriverWait(driver, 10).until(
        expected_conditions.element_to_be_clickable((By.XPATH, "//input[@name='checkAll']"))
    )
    if not checkbox.is_selected():
        driver.execute_script("arguments[0].scrollIntoView(true);", checkbox)
        checkbox.click()
    # Delete selected cards.
    button = WebDriverWait(driver, 10).until(expected_conditions.element_to_be_clickable((By.ID, "deleteSelected")))
    button.click()
    ...
    button = WebDriverWait(driver, 10).until(
        expected_conditions.element_to_be_clickable((By.XPATH, "//a[contains(@href,'/AddDeckList')]"))
    )
    button.click()
    # Wait for page to load.
    textarea = WebDriverWait(driver, 10).until(expected_conditions.presence_of_element_located((By.ID, "AddDecklist")))
...
cart_overview_elem = WebDriverWait(driver, 10).until(
    expected_conditions.presence_of_element_located((By.CSS_SELECTOR, ".cart-overview"))
)
...
WebDriverWait(driver, 10).until(
    expected_conditions.presence_of_element_located((By.CSS_SELECTOR, "section.shipment-block"))
)
...
if len(results_overall_summaries) > 0:
    ...
    for key in sorted(keys):
        ...
        diff = None
        try:
            diff = None if orig_val is None or final_val is None else final_val - orig_val
        except OSError, RuntimeError, ValueError:
            diff = None
